...
with DAG(
    ...
    # Consulta la API REST de HDFS (WebHDFS) para comprobar que el directorio
    # del Data Warehouse fue creado por Spark en el namenode.
    # Esto confirma que el job escribió las tablas en Hive correctamente.
    # =========================================================================
    def verificar_hive(**kwargs):
        ...
        warehouse_db = "/user/hive/warehouse/restaurante_olap.db"
        url = f"{base_url}{warehouse_db}?op=LISTSTATUS"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                tablas = [f["pathSuffix"] for f in archivos]
                print(f"Hive Data Warehouse OK. Tablas en HDFS: {tablas}")
                esperadas = {"tendencias_consumo", "horarios_pico", "crecimiento_mensual"}
                encontradas = set(tablas)
                faltantes = esperadas - encontradas
                if faltantes:
                    print(f"AVISO: Tablas no encontradas en HDFS: {faltantes}")
                else:
                    print("Las 3 tablas OLAP fueron creadas correctamente.")
            elif response.status_code == 404:
                print("AVISO: El directorio 'restaurante_olap.db' no existe en HDFS.")
                print("Puede que el job de Spark no haya completado la escritura en Hive.")
            else:
                print(f"Respuesta inesperada de WebHDFS: HTTP {response.status_code}")
