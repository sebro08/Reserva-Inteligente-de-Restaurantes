...
class AuthController {
    ...
    static async getAdminToken() {
        ...
        const response = await axios_1.default.post(`${KEYCLOAK_URL}/realms/master/protocol/openid-connect/token`, params, { headers: { "Content-Type": "application/x-www-form-urlencoded" } });
    ...
    static async register(req, res) {
        try {
            ...
            // 1. Verificar si el rol existe en PostgreSQL (para que el usuario no lance error al guardar en BD después)
            const role = await roleRepository.findOneBy({ name: role_name });
            if (!role) {
                return res.status(400).json({ message: `El rol ${role_name} no existe en la base de datos local.` });
            }
            ...
            await axios_1.default.post(keycloakUserUrl, newUserPayload, {
                headers: {
            ...
            const usersResponse = await axios_1.default.get(`${keycloakUserUrl}?username=${email}&exact=true`, {
                headers: { Authorization: `Bearer ${adminToken}` }
            });
            const keycloak_id = usersResponse.data[0].id;
            // 5. Asignarle el rol correspondiente dentro de Keycloak
            try {
                const roleResponse = await axios_1.default.get(`${KEYCLOAK_URL}/admin/realms/${REALM}/roles/${role_name}`, {
                    headers: { Authorization: `Bearer ${adminToken}` }
                });
                const roleDef = roleResponse.data;
                await axios_1.default.post(`${keycloakUserUrl}/${keycloak_id}/role-mappings/realm`, [roleDef], {
    ...
    static async login(req, res) {
        try {
            ...
            const response = await axios_1.default.post(`${KEYCLOAK_URL}/realms/${REALM}/protocol/openid-connect/token`, params, { headers: { "Content-Type": "application/x-www-form-urlencoded" } });
